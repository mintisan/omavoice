...
mkdir -p "$stage$data/omavoice" "$stage$data/com.pais.handy" "$stage$home/.local/bin"
printf 'keep\n' >"$stage$data/omavoice/user-data-sentinel"
printf 'keep\n' >"$stage$data/com.pais.handy/handy-data-sentinel"
printf 'keep\n' >"$stage$home/.local/bin/unrelated-sentinel"
printf 'locally modified\n' >"$stage$home/.local/bin/omavoicectl"

"${environment[@]}" "$root/uninstall.sh" --staging-root "$stage" >/dev/null
[[ $(cat "$stage$data/omavoice/user-data-sentinel") == keep ]] || fail 'OmaVoice user data was removed'
...
[[ $(cat "$stage$home/.local/bin/omavoicectl") == 'locally modified' ]] ||
  fail 'a locally modified managed file was removed'
[[ ! -e $stage$home/.local/bin/omavoice-settings ]] || fail 'an unchanged managed file remains'
...
printf 'locally modified\n' >>"$modified_stage$home/.local/lib/Handy/.omavoice-managed"
"${environment[@]}" "$modified_stage$home/.local/bin/omavoicectl" \
...
printf 'unmanaged\n' >"$collision_stage$home/.local/bin/omavoicectl"
...
[[ $(cat "$collision_stage$home/.local/bin/omavoicectl") == unmanaged ]] ||
