...
```bash
Linux/Handy/build-pinned.sh /tmp/sayall-handy-build
```
...
```bash
Linux/Handy/install-user.sh --build-directory /tmp/sayall-handy-build
```

The installer writes only these program files:

- `~/.local/bin/handy`
- `~/.local/bin/sayall-handy` (routes only Handy to the stable ATVVoice source)
- `~/.config/systemd/user/sayall-handy.service`
...
`PIPEWIRE_NODE=atvvoice-sayall-rc003` process environment, so Handy can use the
...
Linux/Handy/install-user.sh \
  --build-directory /tmp/sayall-handy-build \
  --staging-root /tmp/sayall-handy-root
```
