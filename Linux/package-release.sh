...
for c in cargo rustc python3 git sha256sum find file readelf ldd strings cmp cut xargs realpath tar zstd desktop-file-validate systemd-analyze; do command -v "$c" >/dev/null||fail "required command missing: $c"; done
HERE=$(cd -- "$(dirname -- "$0")/.." && pwd -P); manifest=$HERE/Linux/SayAllLinux/Cargo.toml
...
install -m0755 "$atv/target/release/atvvoice" "$root/payload/bin/sayall-atvvoice"
for n in sayall-doctor sayall-settings sayall-statistics sayall-keyd-helper; do [[ -x $HERE/Linux/SayAllLinux/target/release/$n ]]||fail "missing release binary: $n"; done
for n in sayall-doctor sayall-settings sayall-statistics; do install -m0755 "$HERE/Linux/SayAllLinux/target/release/$n" "$root/payload/bin/$n"; done
install -m0755 "$HERE/Linux/sayallctl" "$root/payload/bin/sayallctl"; install -m0755 "$handy/target/release/handy" "$root/payload/bin/handy"; install -m0755 "$HERE/Linux/Handy/sayall-handy" "$root/payload/bin/sayall-handy"
cp -a "$handy/source/src-tauri/transcribe-libs/." "$root/payload/lib/Handy/"; mkdir "$root/payload/lib/Handy/resources"; cp -a "$handy/target/release/resources/." "$root/payload/lib/Handy/resources/"; printf '%s\n' "$H_COMMIT" >"$root/payload/lib/Handy/.sayall-managed"; install -m0644 "$handy/BUILD-METADATA" "$root/payload/lib/Handy/BUILD-METADATA"; install -m0644 "$handy/SHA256SUMS" "$root/payload/lib/Handy/BUILD-SHA256SUMS"
install -m0644 "$HERE"/Linux/systemd/*.service "$HERE/Linux/Handy/sayall-handy.service" "$root/payload/systemd/user/"; install -m0644 "$HERE/Linux/app.sayall.Settings.desktop" "$HERE/Linux/Handy/com.pais.handy.desktop" "$root/payload/share/applications/"
install -m0644 "$HERE/Resources/OmaVoice.png" "$root/payload/share/icons/hicolor/1024x1024/apps/app.sayall.Settings.png"; install -m0644 "$handy/source/src-tauri/icons/128x128@2x.png" "$root/payload/share/icons/hicolor/256x256/apps/handy.png"; install -m0644 "$handy/source/LICENSE" "$root/payload/share/licenses/Handy/LICENSE"
install -m0755 "$HERE/Linux/release/keyd-install.sh" "$root/optional-keyd/install.sh"; install -m0755 "$HERE/Linux/release/keyd-uninstall.sh" "$root/optional-keyd/uninstall.sh"; install -m0755 "$HERE/Linux/SayAllLinux/target/release/sayall-keyd-helper" "$root/optional-keyd/"; install -m0644 "$HERE/Linux/polkit/app.sayall.keyd.policy" "$root/optional-keyd/"
...
for b in "$root"/payload/bin/sayall-atvvoice "$root"/payload/bin/sayall-{doctor,settings,statistics} "$root/payload/bin/handy" "$root/optional-keyd/sayall-keyd-helper"; do file "$b"|grep -q 'ELF 64-bit LSB.*x86-64'||fail "not x86_64 ELF: $b"; for leaked in "$HERE" "$atv" "$handy"; do strings "$b"|grep -Fq "$leaked"&&fail "build path embedded: $b"; done; done
for b in "$root"/payload/bin/sayall-atvvoice "$root"/payload/bin/sayall-{doctor,settings,statistics} "$root/optional-keyd/sayall-keyd-helper"; do readelf -d "$b"|grep -Eq 'RPATH|RUNPATH'&&fail "RPATH present: $b"; done
...
LD_LIBRARY_PATH="$root/payload/lib/Handy" ldd "$root/payload/bin/handy"|grep -q 'not found'&&fail 'Handy has missing runtime dependencies'; for b in "$root"/payload/bin/sayall-atvvoice "$root"/payload/bin/sayall-{doctor,settings,statistics}; do ldd "$b"|grep -q 'not found'&&fail "missing runtime dependency: $b"; done
desktop-file-validate "$root"/payload/share/applications/*; systemd-analyze verify --user "$root"/payload/systemd/user/*.service; bash -n "$root"/{install.sh,uninstall.sh} "$root"/optional-keyd/{install.sh,uninstall.sh}
